...
bowtie2 -x /path/to/hg38_index  -1 $file -2 $name2 -S host_removed/${name}.sam --un-conc host_removed/${name}_host_removed -p 100
mv host_removed/${name}_host_removed.1 host_removed/${name}_host_removed_1.fastq
mv host_removed/${name}_host_removed.2 host_removed/${name}_host_removed_2.fastq
done
...
echo -e "Original_Name\tNew_Name" > rename_mapping.txt  
counter=1  
for file in $(ls *MicrobesPETCork_bin.*.fa | sort -V); do
...
/path/to/${dir}_host_removed_1.fastq \
/path/to/${dir}_host_removed_2.fastq 
